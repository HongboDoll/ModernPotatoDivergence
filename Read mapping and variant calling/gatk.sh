...
do
    ...
    gatk --java-options \"-Xmx10G -XX:ParallelGCThreads=1\" HaplotypeCaller -R $ref --sample-ploidy 4 --emit-ref-confidence GVCF -I mapping/${i}.sort.markdup.bam -O gvcf/${i}.sample.g.vcf.gz --native-pair-hmm-threads 1 # --sample-ploidy assumes samples are polyploidy
    """ > ${i}_gatk.sh
done
...
do
    echo """#!/bin/bash
    gatk --java-options \"-Xmx50G -XX:ParallelGCThreads=7\" GenomicsDBImport -R $ref -L $i --sample-name-map sample.gvcf.xls --genomicsdb-workspace-path ${i}.gatk.db

    gatk --java-options \"-Xmx50G -XX:ParallelGCThreads=7\" GenotypeGVCFs -R $ref --allow-old-rms-mapping-quality-annotation-data --sample-ploidy 4 -V gendb://${i}.gatk.db -O ${i}.gatk.vcf # --sample-ploidy
...
gatk --java-options "-Xmx200G -XX:ParallelGCThreads=70" VariantFiltration -V chr${i}.gatk.snp.vcf.gz \
...
-O chr${i}.gatk.snp.filter.vcf.gz

gatk SelectVariants --max-indel-size 30 --restrict-alleles-to BIALLELIC -select-type INDEL -V chr${i}.gatk.vcf -O chr${i}.gatk.indel.vcf.gz

gatk --java-options "-Xmx200G -XX:ParallelGCThreads=70" VariantFiltration -V chr${i}.gatk.indel.vcf.gz \
--filter-expression "QD < 2.0" --filter-name "LowQD" \
...
--filter-expression "ReadPosRankSum < -8.0" --filter-name "ReadPosRankSum-8.0" \
-G-filter "GQ < 20.0" -G-filter-name "GQ20.0" \
-O chr${i}.gatk.indel.filter.vcf.gz

gatk MergeVcfs \
-I chr${i}.gatk.snp.filter.vcf.gz \
-I chr${i}.gatk.indel.filter.vcf.gz \
-O chr${i}.gatk.filter.vcf

rm chr${i}.gatk.vcf

cat <(grep '#' chr${i}.gatk.filter.vcf | grep -v scaffold) <(grep -v '#' chr${i}.gatk.filter.vcf | grep -v scaffold | awk '$7=="PASS"') > t && mv t chr${i}.gatk.filter.vcf

echo -e \"$PWD/${i}.gatk.filter.vcf\" >> vcf.filter.list

done

gatk MergeVcfs -I vcf.filter.list -O 166_potato_gatk.vcf.gz && rm chr*.gatk.filter.vcf chr*.gatk.vcf

cat <(grep '#' chr${i}.gatk.filter.vcf) <(grep -v '#' chr${i}.gatk.filter.vcf | awk '$7=="PASS"&&$5!~/,/') | bgzip -f > chr${i}.gatk.filter.bi-allelic.vcf.gz

tabix -f -p vcf chr${i}.gatk.filter.vcf.gz
tabix -f -p vcf chr${i}.gatk.filter.bi-allelic.vcf.gz

cat <(grep '#' chr01.gatk.filter.vcf) <(cat chr*.gatk.filter.vcf | grep -v '#') > 166_potato_Allchr.gatk.filter.vcf
