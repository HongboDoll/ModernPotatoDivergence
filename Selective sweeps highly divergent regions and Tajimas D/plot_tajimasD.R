...
plot(1,1,type='n',xaxs='i',yaxs='i',cex=0.1,col='steelblue1',axes=F,main='',xlab='',ylab='FST',xlim=c(0,sum(len)),ylim=c(-4,6))
par(new=T)
...
for (i in 1:chr_num){ #### chromosomes
  ...
  for (n in 1:length(d[,2])){
    plot(1,1,type='n',xaxs='i',yaxs='i',cex=0.1,col='blue',axes=F,main='',xlab='',ylab='',xlim=c(0,sum(len)),ylim=c(-4,6)) # add layer out
#	if (d[n,3] >= 0) {
#	color <- '#01A8D1'
#	} else {
#	color <- '#FFA049'
...
axis(2,tcl=-0.42,lwd=1,lwd.ticks=1,at=seq(-4,6,2),labels=seq(-4,6,2))
