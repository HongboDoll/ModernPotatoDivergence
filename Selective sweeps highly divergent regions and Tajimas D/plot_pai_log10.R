...
plot(1,1,type='n',xaxs='i',yaxs='i',cex=0.1,col='steelblue1',axes=F,main='',xlab='',ylab='Nucleotide diversity',xlim=c(0,sum(len)),ylim=c(0,1))
par(new=T)
...
for (i in 1:chr_num){ #### chromosomes
  ...
  for (n in 1:length(d[,2])){
    plot(1,1,type='n',xaxs='i',yaxs='i',cex=0.1,col='blue',axes=F,main='',xlab='',ylab='',xlim=c(0,sum(len)),ylim=c(0, log(1200,10))) # add layer out
    points(len_sum+d[n,2],log(d[n,3],10),type='h',xaxs='i',yaxs='i',cex=0.2,col=color,pch=20) # plot point
...
axis(2,tcl=-0.3,lwd=2,lwd.ticks=2,at=c(0,log(2,10),log(5,10),log(10,10),log(50,10),log(200,10),log(1000,10)),labels=c(0,2,5,10,50,200,'1,000'))
...
#}
abline(h=log(threshold,10),lty=2,lwd=1.6,col='black')
