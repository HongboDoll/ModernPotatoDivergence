...
qtl <- read.table(argv[5], header=F)
...
plot(1,1,type='n',xaxs='i',yaxs='i',cex=0.1,col='steelblue1',axes=F,main='',xlab='',ylab='fdM',xlim=c(0,sum(len)),ylim=c(0,0.4))
par(new=T)
...
for (i in 1:chr_num){ #### chromosomes
  ...
  for (n in 1:length(d[,2])){
    plot(1,1,type='n',xaxs='i',yaxs='i',cex=0.1,col='blue',axes=F,main='',xlab='',ylab='',xlim=c(0,sum(len)),ylim=c(0,0.4)) # add layer out
...
#    points(len_sum+d[n,2],d[n,7],type='p',xaxs='i',yaxs='i',cex=0.2,col='red',pch=20) # plot point
  ...
  for (n in 1:length(q[,2])){
	...
	segments(len_sum+q[n,2],0.2+(n-1)*0.04,len_sum+q[n,3],0.2+(n-1)*0.04,col='black',lwd=2)
	text(len_sum+q[n,2], 0.2+(n-1)*0.04+0.02, q[n,4], cex=0.8)
	}
  par(new=T)
}
...
axis(2,tcl=-0.42,lwd=1,lwd.ticks=1,at=seq(0,0.4,0.1),labels=seq(0,0.4,0.1))
