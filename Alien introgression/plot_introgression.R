...
plot(1,1,type='n',xaxs='i',yaxs='i',cex=0.1,col='steelblue1',axes=F,main='',xlab='',ylab='fdM',xlim=c(0,sum(len)),ylim=c(0,0.5))
par(new=T)
...
for (i in 1:chr_num){ #### chromosomes
  ...
  for (n in 1:length(d[,2])){
    plot(1,1,type='n',xaxs='i',yaxs='i',cex=0.1,col='blue',axes=F,main='',xlab='',ylab='',xlim=c(0,sum(len)),ylim=c(0,0.5)) # add layer out
...
axis(2,tcl=-0.42,lwd=1,lwd.ticks=1,at=seq(0,0.5,0.1),labels=seq(0,0.5,0.1))
